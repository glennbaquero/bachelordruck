...
<div x-data="{
    ...
    select(value) {
        ...
        if (!this.model?.toString().length) {
            ...
            this.content = this.getOptionElement(this.model).innerHTML
        }
    },
    clearModel() {
    ...
    getOptionElement(value) {
        if (this.$refs.optionsContainer === undefined) return
    ...
    getValueText() {
        ...
        if (this.getOptionElement(this.model) === undefined) return null
...
>
    <div class="relative">
        ...
        @if ($avatar)
            <div
                x-ref="select"
                class="relative bg-white rounded-md border border-gray-400 border-solid p-1 h-12
                {{ $errors->has($name)
                    ? $errorClasses()
                    : ''
                }}"
    ...
    <div class="absolute w-full mt-1 rounded-lg overflow-hidden shadow-md bg-white z-10 border border-secondary-200
                dark:bg-secondary-800 dark:border-secondary-600 z-20"
         x-show="popover"
         x-cloak
